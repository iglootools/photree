...
class TestRmUpstreamHeic:
    def test_removes_upstream_when_jpeg_deleted(self, tmp_path: Path) -> None:
        ...
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0410.HEIC", "IMG_0410.AAE"])
        _setup_dir(tmp_path / "ios/edit-img", ["IMG_E0410.HEIC", "IMG_O0410.AAE"])
        ...
        assert _names(tmp_path / "ios/edit-img") == set()
        assert _names(tmp_path / "ios/orig-img") == set()
    ...
    def test_keeps_files_with_jpeg_present(self, tmp_path: Path) -> None:
        ...
        assert "IMG_0002.HEIC" not in _names(tmp_path / "ios/orig-img")
        # IMG_0001 files are untouched
        assert "IMG_0001.HEIC" in _names(tmp_path / "ios/orig-img")
        assert "IMG_0001.AAE" in _names(tmp_path / "ios/orig-img")

    def test_nothing_to_remove(self, tmp_path: Path) -> None:
        """No deletions when all jpegs are present."""
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.HEIC"])
    ...
    def test_handles_jpg_copy_as_is(self, tmp_path: Path) -> None:
        ...
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.JPG"])
    ...
    def test_removes_upstream_when_combined_heic_deleted(self, tmp_path: Path) -> None:
        ...
        _setup_dir(tmp_path / "ios/edit-img", ["IMG_E0001.HEIC", "IMG_O0001.AAE"])
        ...
        assert "IMG_0002.HEIC" in _names(tmp_path / "ios/orig-img")
    ...
    def test_dry_run_does_not_modify(self, tmp_path: Path) -> None:
        ...
        result = rm_upstream(tmp_path, dry_run=True)
...
class TestRmUpstreamMov:
    def test_removes_upstream_when_combined_mov_deleted(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-vid", ["IMG_0115.MOV"])
        _setup_dir(tmp_path / "ios/edit-vid", ["IMG_E0115.MOV"])
        ...
        assert _names(tmp_path / "ios/edit-vid") == set()
        assert _names(tmp_path / "ios/orig-vid") == set()

    def test_keeps_files_with_combined_mov_present(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-vid", ["IMG_0001.MOV", "IMG_0002.MOV"])
        _setup_dir(tmp_path / "ios/edit-vid", ["IMG_E0001.MOV"])
        ...
        # IMG_0001 untouched
        assert "IMG_0001.MOV" in _names(tmp_path / "ios/orig-vid")
        assert "IMG_E0001.MOV" in _names(tmp_path / "ios/edit-vid")

    def test_nothing_to_remove(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-vid", ["IMG_0001.MOV"])
...
class TestRmOrphanHeic:
    def test_removes_orphan_rendered_and_combined(self, tmp_path: Path) -> None:
        ...
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_0001.AAE"])
        _setup_dir(
    ...
    def test_dry_run_does_not_modify(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", [])
        _setup_dir(tmp_path / "ios/edit-img", ["IMG_E9999.HEIC"])

        result = rm_orphan(tmp_path, dry_run=True)

        assert result.heic.total > 0
        assert "IMG_E9999.HEIC" in _names(tmp_path / "ios/edit-img")
...
class TestRmOrphanMov:
    def test_removes_orphan_rendered_and_combined_mov(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-vid", ["IMG_0001.MOV"])
        _setup_dir(tmp_path / "ios/edit-vid", ["IMG_E0001.MOV", "IMG_E9999.MOV"])
        _setup_dir(tmp_path / "main-vid", ["IMG_E0001.MOV", "IMG_E9999.MOV"])

        rm_orphan(tmp_path)

        assert "IMG_E9999.MOV" not in _names(tmp_path / "ios/edit-vid")
...
class TestRmOrphanSidecar:
    def test_removes_orphan_aae_in_orig(self, tmp_path: Path) -> None:
        _setup_dir(
            tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_0001.AAE", "IMG_9999.AAE"]
        )
        _setup_dir(tmp_path / "ios/edit-img", [])

        result = rm_orphan_sidecar(tmp_path)
        assert result.total == 1
        assert not (tmp_path / "ios/orig-img" / "IMG_9999.AAE").exists()
        assert (tmp_path / "ios/orig-img" / "IMG_0001.AAE").exists()
    ...
    def test_removes_orphan_o_aae_in_rendered(self, tmp_path: Path) -> None:
        ...
        result = rm_orphan_sidecar(tmp_path)
        assert result.total == 1
        assert not (tmp_path / "ios/edit-img" / "IMG_O9999.AAE").exists()
        assert (tmp_path / "ios/edit-img" / "IMG_O0001.AAE").exists()
    ...
    def test_dry_run_does_not_modify(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_9999.AAE"])
        _setup_dir(tmp_path / "ios/edit-img", [])

        result = rm_orphan_sidecar(tmp_path, dry_run=True)
        assert result.total == 1
        assert (tmp_path / "ios/orig-img" / "IMG_9999.AAE").exists()
...
class TestPreferHigherQualityWhenDups:
    def test_removes_jpg_when_heic_exists(self, tmp_path: Path) -> None:
        _setup_dir(
            tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_0001.JPG", "IMG_0001.AAE"]
        )

        result = prefer_higher_quality_when_dups(tmp_path)

        assert result.total == 1
        assert "IMG_0001.JPG" not in _names(tmp_path / "ios/orig-img")
        assert "IMG_0001.HEIC" in _names(tmp_path / "ios/orig-img")
        assert "IMG_0001.AAE" in _names(tmp_path / "ios/orig-img")

    def test_removes_across_all_image_dirs(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_0001.JPG"])
    ...
    def test_keeps_jpg_without_heic_dup(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_0002.JPG"])

        result = prefer_higher_quality_when_dups(tmp_path)

        assert result.total == 0
        assert "IMG_0002.JPG" in _names(tmp_path / "ios/orig-img")

    def test_removes_png_when_heic_exists(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_0001.PNG"])

        result = prefer_higher_quality_when_dups(tmp_path)

        assert result.total == 1
        assert "IMG_0001.PNG" not in _names(tmp_path / "ios/orig-img")

    def test_nothing_to_remove(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_0002.HEIC"])
    ...
    def test_dry_run_does_not_modify(self, tmp_path: Path) -> None:
        ...
        result = prefer_higher_quality_when_dups(tmp_path, dry_run=True)

        assert result.total == 1
        assert "IMG_0001.JPG" in _names(tmp_path / "ios/orig-img")
...
class TestRmMiscategorized:
    def test_removes_rendered_from_orig(self, tmp_path: Path) -> None:
        ...
        result = rm_miscategorized(tmp_path)

        assert set(result.heic.fixed_from_orig) == {"IMG_E0001.HEIC", "IMG_O0001.AAE"}
        assert "IMG_E0001.HEIC" not in _names(tmp_path / "ios/orig-img")
        assert "IMG_O0001.AAE" not in _names(tmp_path / "ios/orig-img")
        # Originals untouched
        assert "IMG_0001.HEIC" in _names(tmp_path / "ios/orig-img")
        assert "IMG_0001.AAE" in _names(tmp_path / "ios/orig-img")
    ...
    def test_removes_original_from_rendered(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", [])
        _setup_dir(tmp_path / "ios/edit-img", ["IMG_0001.HEIC", "IMG_E0001.HEIC"])

        result = rm_miscategorized(tmp_path)

        assert result.heic.fixed_from_rendered == ("IMG_0001.HEIC",)
        assert "IMG_0001.HEIC" not in _names(tmp_path / "ios/edit-img")
        assert "IMG_E0001.HEIC" in _names(tmp_path / "ios/edit-img")
    ...
    def test_mov_dirs(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-vid", ["IMG_E0001.MOV"])
        _setup_dir(tmp_path / "ios/edit-vid", [])
    ...
    def test_dry_run_does_not_modify(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_E0001.HEIC"])
        _setup_dir(tmp_path / "ios/edit-img", [])

        result = rm_miscategorized(tmp_path, dry_run=True)

        assert result.total == 1
        assert "IMG_E0001.HEIC" in _names(tmp_path / "ios/orig-img")
...
class TestRmMiscategorizedSafe:
    def test_removes_when_present_in_correct_dir(self, tmp_path: Path) -> None:
        ...
        _setup_dir(tmp_path / "ios/orig-img", ["IMG_0001.HEIC", "IMG_E0001.HEIC"])
        _setup_dir(tmp_path / "ios/edit-img", ["IMG_E0001.HEIC"])

        result = rm_miscategorized_safe(tmp_path)
    ...
    def test_both_directions(self, tmp_path: Path) -> None:
        ...
        _setup_dir(tmp_path / "ios/edit-img", ["IMG_E0001.HEIC", "IMG_0099.HEIC"])
        _setup_dir(
            tmp_path / "ios/orig-img", ["IMG_0099.HEIC"]
...
class TestMvMiscategorized:
    def test_moves_rendered_from_orig_to_rendered(self, tmp_path: Path) -> None:
        ...
        result = mv_miscategorized(tmp_path)

        assert set(result.heic.fixed_from_orig) == {"IMG_E0001.HEIC", "IMG_O0001.AAE"}
        # Moved to rendered
        assert "IMG_E0001.HEIC" in _names(tmp_path / "ios/edit-img")
        assert "IMG_O0001.AAE" in _names(tmp_path / "ios/edit-img")
        # Gone from orig
        assert "IMG_E0001.HEIC" not in _names(tmp_path / "ios/orig-img")
        assert "IMG_O0001.AAE" not in _names(tmp_path / "ios/orig-img")
        # Original stays
    ...
    def test_moves_original_from_rendered_to_orig(self, tmp_path: Path) -> None:
        _setup_dir(tmp_path / "ios/orig-img", [])
        _setup_dir(tmp_path / "ios/edit-img", ["IMG_0001.HEIC"])
    ...
    def test_dry_run_does_not_modify(self, tmp_path: Path) -> None:
        ...
        result = mv_miscategorized(tmp_path, dry_run=True)

        assert result.total == 1
        assert "IMG_E0001.HEIC" in _names(tmp_path / "ios/orig-img")
        assert "IMG_E0001.HEIC" not in _names(tmp_path / "ios/edit-img")
